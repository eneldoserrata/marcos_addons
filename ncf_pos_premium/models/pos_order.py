        ...
        if order_id:
            ...
            if order_id.is_return_order:
                ...
                if not order_id.return_order_id.invoice_id:
                    # This can be happend if server restart when invoice queue_job is runing
            ...
            order_id.with_delay(priority=1, channel="root.pos", max_retries=3, eta=10,
                                description="Creando factura desde el POS").action_pos_order_invoice()
            order_id.with_delay(priority=2, channel="root.pos", eta=15,
                                description="Creando picking desde el POS").create_picking_job()
            _logger.info("Pos order sent to queue_job to be invoiced")
            return res
        else:
            return False
